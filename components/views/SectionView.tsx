...
export function SectionView({ lang, section }: { lang: Lang; section: string }) {
  const sec = SECTION_TITLES[section];
  if (!sec) return notFound();
  const pages = getAllPages(lang).filter(
    (p) => p.section === section && !p.hide_from_nav,
  );
  const order = Object.keys(SECTION_TITLES).indexOf(section) + 1;
